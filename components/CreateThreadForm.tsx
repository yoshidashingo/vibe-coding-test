...
export function CreateThreadForm({ boardId }: CreateThreadFormProps) {
  const [title, setTitle] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isExpanded, setIsExpanded] = useState(false)
  const { mutate } = useThreads(boardId)
    ...
    return (
      <div className="mb-8">
        ...
        >
          ...
          <span className="text-gray-600 font-medium">新しいスレッドを作成</span>
        </button>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="card mb-8">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-900">新しいスレッドを作成</h2>
        <button
          type="button"
